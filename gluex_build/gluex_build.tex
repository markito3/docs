...
Outside JLab, when the gluex\_install procedure has been followed (see section \ref{gluex-install}, \bs will be installed under \$GLUEX\_TOP/build\_scripts. You will have to contact the local expert to find the location of \$GLUEX\_TOP. Set your BUILD\_SCRIPTS environment to point to \$GLUEX\_TOP/build\_scripts.

\subsubsection{Install via tar file}

The latest version of \bs is available from GitHub at
\bcenter
\htmladdnormallink{\tt https://github.com/JeffersonLab/build\_scripts/archive/latest.tar.gz}{https://github.com/JeffersonLab/build\_scripts/archive/latest.tar.gz}
\ecenter
...
\section{Descriptions of selected scripts in \bs}

\subsection{\tt osrelease.pl}

osrelease.pl is a Perl script that writes a string to standard output
that identifies the operating system distribution, processor type, and
compiler of the machine upon which it is executed. For example:
\begin{verbatim}
 > osrelease.pl
...
There are four versions of this script, in the following directories, all basically the same:
\bitem\tt
\item \$BUILD\_SCRIPTS
\item \$HALLD\_RECON\_HOME/src/BMS
\item \$HALLD\_SIM\_HOME/src/BMS
\item \$JANA\_HOME/../SBMS
\eitem
