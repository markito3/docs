...
another member of the subgroup. So an invariant subgroup is a
class. Not only that but when each member of the subgroup is
transformed by any one member of the group, the resulting set of
tranformed members recovers the original subgroup. A class need not
have this property, i.e., the result of the transformed class member
needs to end up in the class, but the class need not reproduce itself
after each member is transformed. In this sense the invariant subgroup
is a complete class. The subgroup does not contain members of other
classes since classes are disjoint. The class need not exhaust the
subgroup. The subgroup need only contain all of the members of the
class. The identity is an example of an element which need not be a
member of the class.
