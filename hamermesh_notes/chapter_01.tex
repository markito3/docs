...
$$
after application of the permutation the box/object arrangement ends up as
$$
...
By a group of transformations, $G$, we mean an aggregate of
...
\item if it includes $M$ and $M^\prime$, also includes their composite $MM^\prime$.
\end{enumerate}
...
\genfrac{}{}{0pt}0{\tt 123}{\tt CAB},
$$
and is also equivalent to our example. Going back to the example, applying the
...
boxes $a,...,x$ cycle among themselves. Likewise for the object in
boxes $b,...,y$. I.e.,
$$
(ab)(a...xb...y) = (a...x)(b...y).
$$
...
\begin{itemize}
\item A cycle on $n$ elements is a product of $n-1$ transpositions.
  \begin{itemize}
...
\item If the total number of elements $N$ is even (odd), then the
...
multiplying by $a$ on the left of each member of $H$, where $a$ is a
...
For example, elements of the integers modulo 5 under addition are 0,
1, 2, 3, and 4. The identity is 0. $e$, $a$, $a^2$, $a^3$, and $a^4$
are 0, 1, $1+1 = 2$, $1+1+1 = 3$, and $1+1+1+1 = 4$. $a^5 = 1 + 1 + 1+
1 + 1 = 0 = e$. The inverse of 0 is 0, of 1 is 4, and of 2 is 3. The
transformation of 2 by 3 is $3^{-1} + 2 + 3 = 2 + 2 + 3 = 7\mod 5 =
2$. The transformation of 2 by 4 is $4^{-1} + 2 + 4 = 1 + 2 + 4 =
7\mod 5 = 2$.
...
{\bf Note on definition of conjugate} (p.\ 23) A member of a group is
transformed by another member of the group. Also each member of a
group can be thought of as a transformation. These are two separate
definitions of the word; they apply in two different contexts.
...
\begin{itemize}
  ...
  \item For abelian groups, each element of the group forms a class by itself.
  \item On p.\ 23, $\equiv$ is read ``is conjugate to.''
\end{itemize}

{\bf Gemini on classes.} Is it true that, in group theory, if a member
of a class is transformed by any other member of the the group, the
results is also a member of the class?

Yes, that is correct.

In group theory, a conjugacy class is specifically defined this
way. If you take an element $a$ from a class and ``transform" it
(conjugate it) by any element $g$ in the group—-calculated as
$gag^{-1}$--the resulting element will always belong to that same
class.  Essentially, a class is a set of elements that are all
reachable from one another through this operation.

Is it true that, in group theory, if a member of the group, not a
member of a given class, is transformed by any other member of the the
group, the results is also outside the class?

Yes, that is also true.

Because conjugacy classes are {\it disjoint} (they don't overlap),
every element in a group belongs to exactly one class.  If you start
with an element $b$ that is outside a given class $C$, then $b$ must
belong to some other class $D$. Since any transformation (conjugation)
of $b$ stays within its own class $D$, the result can never ``jump"
into class $C$.

In short:

\begin{itemize}
\item Transforming a member of a class results in a member of {\it that
  same class}.
\item Transforming an element outside that class results in an element
  {\it still outside} that class.
\end{itemize}

{\bf Comment on ``all elements in the same class have the same
  order''} (top of p.\ 24) If the order of $a$ is $h$, then by definition
no lower power
...
resulting set of transformed members recovers the original
...
correspondence which preserves products, but now several elements of
