...
{\bf Restatement of ``adjoin only a two-fold axis. This axis must be at right angles to the $n$-fold
  axis''} (p. 42) If the two-fold axis is at right angles then the
$n$-fold axis is transformed back on itself. If it is not then the
rotated $n$-fold axis must coincide with another $n$-fold axis for the
transformation to be one of symmetry. In other words, there would
have to be more than one $n$-fold axis.

{\bf Question: why are $C^k_n$ and $C^{-k}_n$ in the same class?} (p.\ 43) If the principal axis is two-sided, then the latter can be reached as a transformation of the former by the rotation which transforms the principal axis into its opposite. So they are in the same class by the definition of a class.
