...
Comment, top of p.\ 24, ``all elements in the same class have the same order'': If the order of $a$ is $h$, then by definition no lower power of $h$ gives $e$. So no lover power of $ubu^{-1}$ gives $e$ either. So no lower power of $b$ gives $e$. So $b$ is of order $h$ as well.
